...
const CYAN = "#74b9f1"
const AMBER = "#f3724c"
// This hero is a deliberately dark, immersive video/music showcase by
// design (see the file banner above) and is meant to look the same way
// regardless of the page's normal theme. This project's own design
// tokens (src/index.css) are plain hex values, not the `H S% L%` triples
// `hsl(var(--x))` expects, so reading them through hsl() would produce
// invalid CSS here anyway — these are fixed, standalone values instead
// of a host-theme lookup.
const bgVar = "#05060a"
const fgVar = "#f8fafc"
const fgMutedVar = (a: number) => `rgba(248, 250, 252, ${a})`
const cardVar = (a = 1) => `rgba(20, 24, 31, ${a})`
const ROW_HEIGHT = 60
...
export default function MusicHero({
  ...
  if (isCoarsePointer) {
    return (
      // eslint-disable-next-line jsx-a11y/no-noninteractive-element-interactions -- role="application" deliberately opts this region out of normal document semantics for its own keyboard model (arrow keys, space), per WAI-ARIA authoring practices
      <div
        className={`mh-focusable${className ? ` ${className}` : ""}`}
        onKeyDown={handlePlayerKeyDown}
        // eslint-disable-next-line jsx-a11y/no-noninteractive-tabindex -- same role="application" widget, needs to be focusable for its keyboard model
        tabIndex={0}
        role="application"
...
function SeamlessLoopVideo({
  ...
  return (
    <>
      {/* eslint-disable jsx-a11y/media-has-caption -- decorative, wordless ambient loop with no dialogue to caption; already aria-hidden so assistive tech skips it entirely */}
      <video aria-hidden="true" ref={aRef} src={src} playsInline preload="auto" style={{ ...base, ...style, opacity: aOpacity }} />
      <video aria-hidden="true" ref={bRef} src={src} playsInline preload="auto" style={{ ...base, ...style, opacity: bOpacity }} />
