...
export default function HomePage() {
  return (
    <div className="bg-background">
      {/* Hero. Light minimalist-ui surface, consistent with Header/Sidebar
          — see index.css for the neutral-scale pass. A low-opacity primary
          glow plus grain keep the field from reading as flat. */}
      <section className="relative overflow-hidden">
        <div
          aria-hidden="true"
          className="pointer-events-none absolute -top-40 right-[-10%] size-[560px] rounded-full bg-primary/8 blur-[140px]"
        />
        <div aria-hidden="true" className="bg-noise pointer-events-none absolute inset-0 opacity-[0.025]" />
        ...
        <div className="relative mx-auto max-w-5xl px-6 pt-16 pb-20 sm:pt-24 sm:pb-24">
          <div className="grid gap-12 md:grid-cols-5 md:items-center md:gap-12">
            <div className="md:col-span-3">
              <Badge
                variant="outline"
                className="border-primary/25 bg-primary/8 tracking-wide text-primary uppercase"
              >
                SIH26139 &middot; Hybrid quantum ML screening
              </Badge>
              <h1 className="mt-5 font-serif text-5xl font-semibold tracking-tight text-balance text-foreground sm:text-6xl lg:text-7xl">
              ...
              <div className="mt-8 flex flex-col gap-4 sm:flex-row sm:items-center">
                <Button asChild size="lg" className="group pr-2 pl-6">
            ...
            <div className="md:col-span-2">
              <div className="mx-auto max-w-xs rounded-[2rem] border border-border bg-secondary/40 p-8">
  ...
  return (
    <section aria-labelledby="how-it-works-heading" className="border-t border-border">
      <div ref={ref} className="mx-auto max-w-5xl px-6 py-20 sm:py-24">
        <Badge
          variant="outline"
          className="border-border bg-secondary tracking-wide text-muted-foreground uppercase"
        >
          The pipeline
        </Badge>
        <h2
          id="how-it-works-heading"
          className="mt-4 font-serif text-2xl font-semibold tracking-tight text-balance text-foreground sm:text-3xl"
        >
          How it works
        </h2>
        ...
        <ol className="mt-10 grid gap-4 sm:grid-cols-2">
          {STEPS.map((step, index) => (
            <li
              ...
              className={cn(
                "rounded-lg border border-border bg-card p-5 shadow-soft transition-[opacity,transform,filter,background-color] duration-700 ease-spring hover:bg-secondary/60",
...
function ProjectSection() {
  ...
  return (
    <section
      ...
      ref={ref}
      className={cn(
        "mx-auto max-w-5xl border-t border-border px-6 py-20 transition-[opacity,transform,filter] duration-700 ease-spring sm:py-24",
        revealed ? "translate-y-0 opacity-100 blur-none" : "translate-y-7 opacity-0 blur-sm",
      )}
    >
      <h2
        id="project-heading"
        className="font-serif text-2xl font-semibold tracking-tight text-balance text-foreground sm:text-3xl"
      >
